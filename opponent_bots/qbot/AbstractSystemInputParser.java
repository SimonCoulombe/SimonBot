...
public abstract class AbstractSystemInputParser extends AbstractSystemInputReader {
    ...
    public void timePeriod(String title, boolean start)
    {
    	if (timing)
    	{
	        ...
	        if (!start)
	        	System.err.println(": " + title + " " + (time2-time1));
	    	time1 = time2;
    	}
    }
    ...
    public void processLine(String line) {
        ...
        if (line.equals(READY)) {
            parseSetup(input);
        	timePeriod("parseSetup", false);
            doTurn();
        	timePeriod("doTurn", false);
            finishTurn();
        	timePeriod("finishTurn", false);
            input.clear();
        } else if (line.equals(GO)) {
            parseUpdate(input);
        	timePeriod("parseUpdate", false);
            doTurn();
        	timePeriod("doTurn", false);
            finishTurn();
        	timePeriod("finishTurn", false);
    ...
    public void parseUpdate(List<String> input) {
        ...
        for (String line : input) {
            ...
            */
            String token = toks[0].toUpperCase();
            /*
            ...
            */
            try
            {
	            ...
	            int row = Integer.parseInt(toks[1]); //scanner.nextInt();
	            int col = Integer.parseInt(toks[2]); //scanner.nextInt();
	            switch (updateToken) {
	                case W:
	                    addWater(row, col);
	                break;
	                case A:
	                    ...
	                    {
	                        addAnt(row, col, Integer.parseInt(toks[3])); //scanner.nextInt());
	                    }
	                break;
	                case F:
	                    addFood(row, col);
	                ...
	                case D:
	                    //if (scanner.hasNextInt()) 
	                    {
	                        removeAnt(row, col, Integer.parseInt(toks[3])); //scanner.nextInt());
	                    }
	                break;
	                case H:
	                    //if (scanner.hasNextInt()) 
	                    {
	                        addHill(row, col, Integer.parseInt(toks[3])); //scanner.nextInt());
	                    }
	                break;
	            }
            }
